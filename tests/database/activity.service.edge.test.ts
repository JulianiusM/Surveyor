...
describe('Edge cases & additional scenarios', () => {
    ...
    test('ensureAssignment reuses existing assignment for same slot/user and for guest', async () => {
        ...
        await addActivitySlot('planA', {id: 's1', title: 'S1', day: '2025-01-01', pos: 1, maxAssignees: 2});

        const u1 = await ensureAssignment('s1', 1, null);
        const u2 = await ensureAssignment('s1', 1, null);
        ...
        const g1 = await ensureAssignment('s1', null, 1);
        const g2 = await ensureAssignment('s1', null, 1);
    ...
    test('assignRole is idempotent for same (assignment, role)', async () => {
        await createActivityPlan('planB', 1, 'T', 'D', '2025-01-01', '2025-01-02', true, true);
        await addActivitySlot('planB', {id: 's1', title: 'S', day: '2025-01-01', pos: 1, maxAssignees: 2});
        await ensureRoleId('lead');
        const aid = await ensureAssignment('s1', 1, null);
    ...
    test('doUnassignRole returns false for unknown role; removing one of multiple roles preserves assignment', async () => {
        ...
        await addActivitySlot('planC', {id: 's1', title: 'S', day: '2025-01-01', pos: 1, maxAssignees: 2});
        await ensureRoleId('lead');
        ...
        const roles = await AppDataSource.getRepository(ActivityAssignmentRole).find({where: {assignmentId: aid}});
    ...
    test('createActivityPlanTx with empty slots inserts only the plan', async () => {
        ...
        const slotCount = await AppDataSource.getRepository(ActivitySlot).count({where: {planId: id}});
        expect(slotCount).toBe(0);
    });

    test('reorderActivitySlots does not affect slots from other plans', async () => {
        await createActivityPlan('A', 1, 'T', 'D', '2025-01-01', '2025-01-01', true, true);
        await createActivityPlan('B', 1, 'T', 'D', '2025-01-01', '2025-01-01', true, true);

        await addActivitySlot('A', {id: 'a1', title: 'A1', day: '2025-01-01', pos: 1, maxAssignees: 1});
        await addActivitySlot('B', {id: 'b1', title: 'B1', day: '2025-01-01', pos: 1, maxAssignees: 1});

        // Try to update b1 via reorder on plan A: should be ignored
        await reorderActivitySlots('A', [{slotId: 'b1', pos: 5}]);

        const a1 = await AppDataSource.getRepository(ActivitySlot).findOneByOrFail({id: 'a1'});
        const b1 = await AppDataSource.getRepository(ActivitySlot).findOneByOrFail({id: 'b1'});
        expect(a1.pos).toBe(1);
        expect(b1.pos).toBe(1);
    });

    test('getLastActivitySlotNumber returns 0 when none exist for that day', async () => {
        await createActivityPlan('planX', 1, 'T', 'D', '2025-01-01', '2025-01-02', true, true);
        const max = await getLastActivitySlotNumber('planX', '2025-01-01');
        expect(max).toBe(0);
    });

    test('updateActivitySlot can update just one field (pos) without touching others', async () => {
        await createActivityPlan('planY', 1, 'T', 'D', '2025-01-01', '2025-01-02', true, true);
        await addActivitySlot('planY', {
            id: 's1',
            title: 'Title',
            description: 'Desc',
            day: '2025-01-01',
        ...
        const ok = await updateActivitySlot('s1', {pos: 10});
        expect(ok).toBe(true);

        const s1 = await AppDataSource.getRepository(ActivitySlot).findOneByOrFail({id: 's1'});
    ...
    test('getActivitySlotAssignees returns both user and guest entries with their roles', async () => {
        await createActivityPlan('planZ', 1, 'T', 'D', '2025-01-01', '2025-01-02', true, true);
        await addActivitySlot('planZ', {id: 's1', title: 'S', day: '2025-01-01', pos: 1, maxAssignees: 3});
        ...
        const aUser = await ensureAssignment('s1', user.id, null);
        await assignRole(aUser, 'lead');
        const aGuest = await ensureAssignment('s1', null, guest.id);
        await assignRole(aGuest, 'helper');

        const map = await getActivitySlotAssignees('planZ');
        expect(map['s1']).toHaveLength(2);
        const names = map['s1'].map(x => x.name).sort();
        expect(names).toEqual(['guestB', 'userA']);
    });

    test('getActivityPlanParticipants groups by COALESCE(user.username, guest.username)', async () => {
        await createActivityPlan('planG', 1, 'T', 'D', '2025-01-01', '2025-01-02', true, true);
        await addActivitySlots('planG', [
            {id: 's1', title: 'A', day: '2025-01-01', pos: 1, maxAssignees: 1},
            {id: 's2', title: 'B', day: '2025-01-02', pos: 1, maxAssignees: 1},
        ]);
        ...
        const a1 = await ensureAssignment('s1', user.id, null);
        await assignRole(a1, 'alpha');
        const a2 = await ensureAssignment('s2', null, guest.id);
        await assignRole(a2, 'beta');

        const rows = await getActivityPlanParticipants('planG');
    ...
    test('deleteActivitySlotAssignment returns DeleteResult with affected=1', async () => {
        ...
        await addActivitySlot('planD', {id: 's1', title: 'S', day: '2025-01-01', pos: 1, maxAssignees: 1});
        const id = await ensureAssignment('s1', 1, null);
