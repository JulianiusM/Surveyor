...
export class EventInvoiceShare {
    ...
    @Column("decimal", {name: "base_share_amount", precision: 10, scale: 2})
    baseShareAmount!: string;

    @Column("decimal", {name: "extra_amount", precision: 10, scale: 2, default: "0.00"})
    extraAmount!: string;

    // Store the fully burdened total (base + extras) that the payer owes
    @Column("decimal", {name: "share_amount", precision: 10, scale: 2})
    shareAmount!: string;
