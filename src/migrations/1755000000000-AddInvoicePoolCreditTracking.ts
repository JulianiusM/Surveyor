...
export class AddInvoicePoolCreditTracking1755000000000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Make migration idempotent by checking if column exists before adding
        const hasCreditAmount = await queryRunner.hasColumn("event_invoice_pools", "credit_amount");
        if (!hasCreditAmount) {
